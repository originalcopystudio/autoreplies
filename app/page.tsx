...
export const metadata: Metadata = {
  title: "AutoReplies - Open source Instagram comment-to-DM automation",
  description:
...
const GITHUB_URL = "https://github.com/originalcopystudio/autoreplies";
const SETUP_DOCS_URL =
  "https://github.com/originalcopystudio/autoreplies/blob/main/docs/setup.md";
    ...
    <AppWindow label="app / overview">
      ...
      <div className="mt-4 rounded border border-border bg-surface p-4">
        ...
        <table className="mt-3 w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left text-[11px] uppercase tracking-wide text-[#8A8375]">
          ...
          <tbody>
            {overviewPosts.map(([post, views, likes, date]) => (
              <tr key={post} className="border-b border-border last:border-0">
                ...
                <td className="py-2 pl-3 text-right text-[#8A8375]">{date}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="mt-4 flex h-32 items-end gap-2">
          {dashboardChart.map(([day, n]) => (
            <div key={day} className="flex flex-1 flex-col items-center gap-2">
              ...
              />
              <span className="text-[10px] text-[#8A8375]">{day}</span>
            </div>
          ))}
        </div>
      </div>
...
async function getGitHubStars(): Promise<number | null> {
  try {
    const res = await fetch("https://api.github.com/repos/originalcopystudio/autoreplies", {
...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#141210] text-[#F5F1E8]">
      <DemoNotice variant="banner" />

      <header className="sticky top-0 z-40 border-b border-[#2C2720] bg-[#141210]">
        <div className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-5 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 text-sm font-semibold text-[#B3AC9D] transition hover:text-[#F5F1E8]"
              aria-label="View AutoReplies on GitHub"
            >
            ...
            <Link
              href="/login"
              className="inline-flex items-center justify-center gap-2 bg-[#F0A24A] px-4 py-2 text-sm font-bold text-[#141210] transition hover:bg-[#F7B96B]"
            >
              Get started
            </Link>
          </div>
      ...
      <section className="mx-auto grid w-full max-w-6xl items-center gap-10 px-5 pb-16 pt-12 sm:px-6 sm:pt-18 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:pb-24">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 border border-[#2C2720] bg-[#141210] px-3 py-2 text-sm font-semibold text-[#B3AC9D]">
            Open source · Official Meta API
          </div>

          <h1 className="mt-7 text-balance text-5xl font-black leading-[1.02] text-[#F5F1E8] sm:text-6xl lg:text-7xl">
            Make every comment start the right DM
          </h1>

          <p className="mt-6 max-w-2xl text-lg leading-8 text-[#B3AC9D]">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link
              href="/login"
              className="inline-flex items-center justify-center gap-2 bg-[#F0A24A] px-6 py-3 text-sm font-bold text-[#141210] transition hover:bg-[#F7B96B]"
            >
              Get started
            </Link>
            <a
              href="#how"
              className="inline-flex items-center justify-center border border-[#2C2720] bg-[#141210] px-6 py-3 text-sm font-bold text-[#F5F1E8] transition hover:border-zinc-300 hover:bg-[#26211A]"
            >
              See how it works
            </a>
          </div>
          ...
          <dl className="mt-10 grid max-w-xl grid-cols-3 gap-3">
            {heroStats.map((stat) => (
              <div key={stat.label} className="border border-[#2C2720] bg-[#1C1916] p-4">
                <dt className="text-2xl font-black text-[#F5F1E8]">{stat.value}</dt>
                <dd className="mt-1 text-xs leading-5 text-[#8A8375]">{stat.label}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      <section id="how" className="mx-auto w-full max-w-6xl px-5 py-20 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
          <div>
            <p className="text-sm font-bold uppercase text-[#F0A24A]">How it works</p>
            <h2 className="mt-3 text-4xl font-black leading-tight text-[#F5F1E8] sm:text-5xl">
              A comment in, a DM out
            </h2>
            <p className="mt-5 text-base leading-8 text-[#B3AC9D]">
          ...
          <div className="grid gap-4">
            {flowSteps.map((step) => (
              <article
                key={step.title}
                className="grid gap-4 border border-[#2C2720] bg-[#1C1916] p-5 sm:grid-cols-[120px_1fr]"
              >
                <p className="text-sm font-bold text-[#F0A24A]">{step.eyebrow}</p>
                <div>
                  <h3 className="text-xl font-bold text-[#F5F1E8]">{step.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-[#B3AC9D]">{step.description}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="border-y border-[#2C2720] bg-[#1C1916] py-20">
        <div className="mx-auto grid w-full max-w-6xl gap-10 px-5 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:px-8 lg:items-center">
          <DashboardPreview />

          <div>
            <p className="text-sm font-bold uppercase text-[#F0A24A]">The dashboard</p>
            <h2 className="mt-3 text-4xl font-black leading-tight text-[#F5F1E8] sm:text-5xl">
      ...
      <section id="features" className="mx-auto w-full max-w-6xl px-5 py-20 sm:px-6 lg:px-8">
        <div className="max-w-2xl">
          <p className="text-sm font-bold uppercase text-[#F0A24A]">What&rsquo;s included</p>
          <h2 className="mt-3 text-4xl font-black leading-tight text-[#F5F1E8] sm:text-5xl">
            Everything, no tiers
          </h2>
          <p className="mt-5 text-base leading-8 text-[#B3AC9D]">
        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => (
            <div
              key={feature}
              className="border border-[#2C2720] bg-[#1C1916] p-4 text-sm font-semibold text-[#D8D2C4]"
            >
              {feature}
            </div>
          ))}
      ...
      <section className="mx-auto w-full max-w-6xl px-5 pb-20 sm:px-6 lg:px-8">
        <div className="grid gap-8 border border-orange-200 bg-[#F0A24A]/10 p-6 sm:p-10 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <h2 className="max-w-3xl text-4xl font-black leading-tight text-[#F5F1E8] sm:text-5xl">
              Turn your next reel&rsquo;s comments into DMs
            </h2>
            <p className="mt-4 text-base text-[#B3AC9D]">
              Free and open source. Star it if it saves you a subscription.
            </p>
            <p className="mt-3 text-sm leading-6 text-[#B3AC9D]">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="font-bold text-[#E8933A] underline underline-offset-2 transition hover:text-orange-800"
              >
                setup guide
              </a>{" "}
      ...
      <footer className="border-t border-[#2C2720] py-8">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 text-sm text-[#8A8375] sm:px-6 lg:px-8">
          <span className="font-semibold text-[#B3AC9D]">AutoReplies</span>
          <a
            href={GITHUB_URL}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 transition hover:text-[#F5F1E8]"
          >
            <svg
              viewBox="0 0 16 16"
              aria-hidden="true"
