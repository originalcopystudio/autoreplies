...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Serverless port: there is no resident worker, so drain due jobs inline —
    // the commenter still gets their DM within this same invocation. Delayed
    // jobs (read fallbacks, follow-ups, requeues) stay queued for the cron.
    // A drain failure must not fail the webhook: rows stay PENDING and the
    // process-queue cron retries them minutes later.
    try {
      await drainQueue();
    } catch (drainError) {
      console.error(
        "[Webhook] Inline drain failed (cron will retry):",
