...
export async function generateMetadata({
  ...
  if (!template) {
    return {
      title: "Template Not Found - AutoReplies",
    };
  }
...
export default async function TemplateDetailPage({ params }: TemplatePageProps) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="border-b border-white/10 bg-[#0E0C0A]/55">
        <div className="mx-auto grid w-full max-w-7xl gap-10 px-5 py-14 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-20">
          <div>
            ...
            <p className="mt-8 text-sm font-bold uppercase tracking-wide text-[#F0A24A]">
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-8 px-5 py-16 sm:px-6 lg:grid-cols-[0.78fr_1.22fr] lg:px-8">
        ...
        <div id="playbook" className="space-y-8">
            ...
            <ol className="mt-5 space-y-3">
              {template.playbook.map((step, index) => (
                <li key={step} className="grid gap-3 sm:grid-cols-[40px_1fr]">
                  <span className="flex h-8 w-8 items-center justify-center bg-[#F0A24A] text-sm font-black text-[#141210]">
                    {index + 1}
                  </span>
          ...
          <section className="border border-[#F0A24A]/25 bg-[#F0A24A]/10 p-6">
            <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                <h2 className="text-2xl font-black text-white">
                  Copy this campaign into AutoReplies
                </h2>
      ...
      <section className="border-t border-white/10 bg-[#0E0C0A]/60 py-14">
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {relatedTemplates.map((item) => (
              ...
              >
                <p className="text-xs font-semibold uppercase tracking-wide text-[#F0A24A]">
                  {item.category}
                </p>
