...
// Serverless port: jobs are plain rows run by lib/queue/drain.ts, not BullMQ
// jobs run by a resident worker. JobLike preserves the fields this logic reads
// (data, name, attemptsMade).
type Job<T> = JobLike<T>;
...
/**
 * Route a queue job to its processor. This is the whole worker now: called by
 * lib/queue/drain.ts for each claimed row (webhook-inline for immediate jobs,
 * cron for delayed ones). Failure recording/retry lives in drain.ts.
 */
