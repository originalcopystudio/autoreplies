...
export async function exchangeCodeForToken(
  ...
  // Instagram returns either a flat object or {"data":[{...}]} depending on
  // the app/flow; unwrap both. A missing token must fail HERE with the real
  // response — passing undefined downstream surfaces as a cryptic code-100
  // "Unsupported request" on the long-lived exchange instead.
  const data = Array.isArray(raw?.data) ? raw.data[0] : raw;
  if (!data?.access_token || data?.user_id == null) {
    throw new Error(
      `Token exchange returned no access_token: ${JSON.stringify(raw).slice(0, 300)}`
    );
  }
  return {
