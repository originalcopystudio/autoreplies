...
// Instagram's token exchange/refresh endpoints are UNVERSIONED: calling them
// with a /vXX.X/ prefix fails with code 100 "Unsupported request" (IGApiException).
const INSTAGRAM_GRAPH_UNVERSIONED = "https://graph.instagram.com";

export async function getLongLivedToken(
  shortLivedToken: string
): Promise<{ accessToken: string; expiresIn: number }> {
  const url = new URL(`${INSTAGRAM_GRAPH_UNVERSIONED}/access_token`);
  ...
  try {
    ...
    // Newer Instagram Business Login flows return an ALREADY long-lived token
    // from the code exchange, and ig_exchange_token on such a token is refused
    // with code 100 "Unsupported request". Use the token as-is: the callback
    // validates it against the profile endpoint right after, and the daily
    // refresh cron (ig_refresh_token) extends it like any long-lived token.
    const message = err instanceof Error ? err.message : "";
    if (/unsupported\s+request/i.test(message) && shortLivedToken?.length > 100) {
      console.warn(
        "[Instagram] ig_exchange_token refused; treating login token as long-lived"
      );
...
export async function refreshLongLivedToken(
  ...
  const url = new URL(`${INSTAGRAM_GRAPH_UNVERSIONED}/refresh_access_token`);
