...
export default function SeoPageShell({ config }: { config: SeoPageConfig }) {
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="border-b border-white/10 bg-[#0E0C0A]/70">
        <div className="mx-auto grid w-full max-w-7xl gap-10 px-5 py-20 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8">
          <div>
            <p className="text-sm font-bold uppercase text-[#F0A24A]">
              {config.eyebrow}
            </p>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/login"
                className="inline-flex items-center justify-center bg-[#F0A24A] px-6 py-3 text-sm font-bold text-[#141210] transition hover:bg-[#F7B96B]"
              >
                {config.primaryCta}
              </Link>
              <Link
      ...
      <section className="border-y border-white/10 bg-white/[0.025] py-16">
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 overflow-hidden border border-white/10">
            <div className="grid grid-cols-[0.8fr_1fr_1fr] border-b border-white/10 bg-[#0E0C0A] text-xs font-bold uppercase tracking-wide text-zinc-500">
              <div className="p-4">Need</div>
              <div className="p-4 text-[#F2D8B4]">AutoReplies</div>
            ...
            {config.comparisons.map((item) => (
              ...
              >
                <div className="bg-[#0E0C0A]/50 p-4 text-sm font-semibold text-white">
                  {item.label}
                </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {config.templateLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="border border-white/10 bg-white/[0.035] p-5 text-sm font-semibold text-white transition hover:border-[#F0A24A]/40 hover:bg-[#F0A24A]/10"
            >
              {link.label}
            </Link>
          ))}
      ...
      <section className="border-t border-white/10 bg-[#0E0C0A]/70 py-16">
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
          <div>
            <p className="text-sm font-bold uppercase text-[#F0A24A]">FAQ</p>
            <h2 className="mt-3 text-4xl font-black text-white">
      ...
      <section className="mx-auto w-full max-w-7xl px-5 py-16 sm:px-6 lg:px-8">
        <div className="border border-[#F0A24A]/25 bg-[#F0A24A]/10 p-8 text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-zinc-300">
            AutoReplies is built for Instagram professional accounts, official
            Meta private replies, and campaign reporting teams can show clients.
          </p>
          <Link
            href="/login"
            className="mt-8 inline-flex items-center justify-center bg-[#F0A24A] px-6 py-3 text-sm font-bold text-[#141210] transition hover:bg-[#F7B96B]"
          >
            Start free
          </Link>
        </div>
